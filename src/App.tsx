...
export default function App() {
  ...
  const chartsParsed = controller.charts?.length ?? 0
  ...
  return (
    ...
    >
      ...
      >
        <strong>Use a real Excel-generated chart file.</strong> Pick one below.
        The bundled <code>sample-chart.xlsx</code> is generated by openpyxl and
        the engine does <em>not</em> parse its chart (
        <code>controller.charts.length === 0</code>), so it does not exercise
        the bug — it is only a placeholder.
      </p>
      ...
      <p style={{ fontSize: 14 }}>
        Loaded: <code>{sourceName}</code> — charts parsed by engine:{' '}
        <strong style={{ color: chartsParsed > 0 ? '#237804' : '#cf1322' }}>
          {chartsParsed}
        </strong>
        {chartsParsed === 0 && ' (this file will not exercise the chart bug)'}
      </p>
